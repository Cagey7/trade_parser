...
def generate_data_for_doc(conn, year, country, region):
    ...
    if month == 0:
        exit(f"Нет данных по {year} {country} {region}")

    data_for_doc = {}

    # Заголовок
    data_for_doc["document_header"] = f"Взаимная торговля {region_cases[region]["родительный"]} с {country_cases[country]["творительный"]} за {month_ranges[month]}{year} {"год" if month == 12 else "года"}"
    ...
    data_for_doc["summary_text"] = gen_summary_text(summary_data[1], country, month, year, region, summary_data[0][0].split()[0])
    data_for_doc["summary_header"] = f"Показатели взаимной торговли {region_cases[region]["родительный"]} с {country_cases[country]["творительный"]}"
    ...
    data_for_doc["export_header"] = f"Таблица 1 – Основные товары экспорта {region_cases[region]["родительный"]} в {country_cases[country]["винительный"]}"
    data_for_doc["import_header"] = f"Таблица 2 – Основные товары импорта {region_cases[region]["родительный"]} в {country_cases[country]["винительный"]}"
